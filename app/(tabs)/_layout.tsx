...
function ProfileDashboard({ onEdit }: { onEdit: () => void }) {
    ...
    const stats = [
        { label: 'Calories', value: `${consumedCalories ?? 0}`, sub: `/ ${dailyCalories ?? 2000} kcal`, color: '#4CAF50' },
                ...
                <Animated.View style={{ opacity: fadeAnim }}>
                    ...
                    <View style={dashStyles.header}>
                        ...
                        <View style={dashStyles.headerInfo}>
                            <Text style={dashStyles.name}>{fullName || "User"}</Text>
                    ...
                    <View style={dashStyles.xpCard}>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                            <Text style={dashStyles.xpLabel}>⭐ Level {level ?? 1} progress</Text>
                            <Text style={dashStyles.xpCount}>{xp ?? 0} / 1000 XP</Text>
                        </View>
...
function ProfileEditor({ onCancel, isRegistration }: ProfileEditorProps) {
        ...
        >
            ...
            >
                <View style={editStyles.header}>
                    {/* If this is your first registration, enter "Personal account" as shown in the screenshot */}
                ...
                {/* If this is a registration, we show one big green calculation button */}
                {isRegistration ? (
                    <TouchableOpacity style={editStyles.calculateBtn} onPress={handleSubmit(onSubmit)}>
...
export default function ProfileScreen() {
    const { fullName, weight, height } = useUserStore();
    const [isEditing, setIsEditing] = useState(false);
    
    const isRegistered = !!fullName && !!weight && !!height;

    // If you are not registered OR you clicked the Edit button, we display the registration/editing form
    ...
    // If there is data, we show the full profile.
    return <ProfileDashboard onEdit={() => setIsEditing(true)} />;
}
...
const dashStyles = StyleSheet.create({
    ...
    logoutBtn: {
        ...
        borderWidth: 1.5, borderColor: '#FFEBEE', marginBottom: 20
    },
...
const editStyles = StyleSheet.create({
    ...
    title: { fontSize: 28, fontWeight: '700', color: '#1A1A1A', marginBottom: 6, textAlign: 'center' },
    ...
    calculateBtn: {
        backgroundColor: '#65A765', height: 56, borderRadius: 12,
